/************Copyright(C) Kaikai Technology 2018-05-14***********************
File name  : olddecrypt.c
Description: 为了兼容旧版标签信息故将旧版软件加密算法引入
Platform   : MDK V5.26.0.0
Version    : V1.0
Author	   : Jason
Create Time: 2019-03-06
Modify	   : 
Modify Time: 
******************************************************************************/
#include "olddecrypt.h"
#include "stdio.h"
#include "string.h"
//const u8 Key[6] = {0xAA,0xF1,0xB9,0xCD,0xC5,0xFD};//57
const u8 Key[6] = {0xAC,0x11,0x69,0xFD,0xFC,0xDF};//04
const u8 CheckTab[256]=
{
	
};
const u8 CheckTab2[256]=
{
	
};

u8 CheckCRC(u8 *src,u8 len)
{
	u8 i;
	u8 sum = 0;
	for(i = 0;i < len;i++)
	{
		sum += src[i];
	}
	sum = CheckTab[sum];
	return sum;
}

u8 CheckCRC2(u8 *src,u8 len)
{
	u8 t;
	u8 sum2 = 0;
	for(t = 0;t < len;t++)
	{
		sum2 += src[t];
	}
	sum2 = CheckTab2[sum2];
	return sum2;
}

u8 check_data(u8 *indata,u8 *outdata)
{
	u8 temp[10] = {0,},i;
	u8 buf[2]={0,};
	temp[0] = *indata++;//head
	for(i=0;i<6;i++)
	{
		temp[1+i] = *indata++ ^ Key[i];//data
	}
	temp[7] = *indata++;//crc1
	temp[8] = *indata++;//crc2
	temp[9] = *indata;//rssi
	buf[0] = CheckCRC(&temp[1],6);
	buf[1] = CheckCRC2(&temp[1],6);
	if((buf[0] == temp[7])&&(buf[1] == temp[8]))
	{
//		for(i=0;i<10;i++)
//		{
//			printf("%02x ",temp[i]);
//		}
		memcpy(outdata,&temp[1],5);
		memcpy(outdata+5,&temp[9],1);
		return true;
	}
	return false;
}

















